...
# Spawn canvas in a new split pane (Windows Terminal or tmux)
bun run src/cli.ts spawn calendar --scenario meeting-picker --config '{...}'

# Close a canvas programmatically
bun run src/cli.ts close calendar-1
```
...
- **Windows**: Uses Windows Terminal (`wt.exe`) with 50/50 vertical split
- **Unix/macOS**: Uses tmux split pane
...
Interactive canvases communicate via IPC (Unix domain sockets on Unix, TCP localhost on Windows).
...
- **Windows**: Windows Terminal (detected via `WT_SESSION` env var)
- **Unix/macOS**: tmux session (detected via `TMUX` env var)
