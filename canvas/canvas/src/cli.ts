...
program
  ...
  .option("--socket <path>", "Socket path (Unix) or port (Windows) for IPC")
  .option("--scenario <name>", "Scenario name (e.g., display, meeting-picker)")
  .action(async (kind = "document", options) => {
    const id = options.id || `${kind}-1`;

    // Load config from file or parse JSON string
    let config: unknown;
...
program
  ...
  .action(async (id: string, options) => {
    const { getConnectionConfig } = await import("./ipc/types");
    const connectionConfig = getConnectionConfig(id);
...
program
  .command("close <id>")
  .description("Close a running canvas")
  .action(async (id: string) => {
    const { getConnectionConfig } = await import("./ipc/types");
    const connectionConfig = getConnectionConfig(id);
    ...
    try {
      ...
      console.log(`Sent close to canvas '${id}'`);
    } catch (err) {
      console.error(`Failed to connect to canvas '${id}':`, err);
    }
  });
